...
plt.boxplot(df_diabet["bmi"]) #boxplot kutu grafiği oluşturur 'bmi' bizim seçtiğimiz indextir
...
def yeniveriyenikarsilastirma (df,column,threshold):
    ...
    filtrelenmis_degerler = df[df[column] <= threshold] #Sıfırın üstünde bir yerde hata varsa onu belirtir
    plt.subplot(1, 2, 2)
    plt.boxplot(filtrelenmis_degerler[column])
...
yeniveriyenikarsilastirma(df_diabet, "s4",0.1) #0.1  üstünde bir yerde hata varsa onu belirtir
"""
