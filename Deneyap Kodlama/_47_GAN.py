...
from matplotlib import pyplot # ayrımcıyı model tanımlama
...
# model eğitimi
def train(g_model, d_model, gan_model, dataset, son_boyut, n_epochs=5000, n_batch=128):
    bat_per_epo = int(dataset.shape[0] / n_batch)
    half_batch = int(n_batch / 2)
    for i in range(n_epochs):
        for j in range(bat_per_epo): X_gercek, y_gercek = ornek_veri(dataset, half_batch)
